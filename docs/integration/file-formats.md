...
Rescored uses three primary file formats for different purposes:
- **MusicXML** - Primary notation format (editing, display, export)
- **MIDI** - Intermediate format (transcription output, playback)
...
- **Industry Standard**: Used by Finale, Sibelius, MuseScore, Dorico, Notion
- **Rich Semantics**: Clefs, key signatures, articulations, lyrics, dynamics
- **Human-Readable**: XML format, easy to debug
- **Preservation**: All notation information preserved (unlike MIDI)
...
<score-partwise version="4.0">
  <work>
    <work-title>Example Score</work-title>
  </work>
...
- **divisions**: Ticks per quarter note (480 is standard)
- **pitch**: Step (A-G), octave (0-9), optional accidental
- **duration**: In division units (480 = quarter note, 240 = eighth, etc.)
- **type**: Visual representation ("whole", "half", "quarter", "eighth", "16th")
...
const musicXML = await fetch('/api/v1/scores/job123').then(r => r.text());
const score = parseScore(musicXML);

// Access notes
score.parts[0].measures.forEach(measure => {
  measure.notes.forEach(note => {
    console.log(`${note.pitch.step}${note.pitch.octave}`);  // e.g., "C4"
  });
});
```
